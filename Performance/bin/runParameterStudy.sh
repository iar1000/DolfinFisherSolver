...
for d in "${diffusion[@]}"; do
	for r in "${reaction[@]}"; do
		...
		bsub -o "parameter-$d-$r" -n 96 -R fullnode -W 24:00 mpirun ./Performance-FisherSolver --diffCoef1 "$d" --reactCoef "$r" --meshname "lh-white-hull-flood-0-1-merge-5-dof-600k." --name "parameter" --type "$TYPE" --newton_tol "$TOL" --krylovnonzero "$KRYLNONZERO"
	done
done
